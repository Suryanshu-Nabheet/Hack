...
class CredentialTester {
    ...
    private async attemptLogin(userId: string, password: string): Promise<boolean> {
    ...
    private async attemptLoginWithInstance(instance: AxiosInstance, userId: string, password: string): Promise<boolean> {
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                // Get login page to refresh session
